  ...
  isCliTutorModeEnabled
    ? <div className="flex items-center justify-between">
      ...
      <button {...props} className={`bg-animate hover-bg-near-white pa2 pointer flex items-center  ${className}`}>
        <StrokeCode {...props} onClick={() => onCliTutorMode(true)} className="dib fill-link pointer"
                    style={{ height: 38 }}/>
      </button>
    </div>
    : <button role="menuitem" className={`bg-animate hover-bg-near-white pa2 pointer flex items-center ${className}`}
              onClick={onClick} {...props}>
      {children}
    </button>
)
